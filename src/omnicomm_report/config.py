...
# Потолок забора треков — максимально БЕЗОПАСНЫЙ: под общим аккаунт-лимитером
# (170/мин), с запасом ≥50/мин для живого 3ч-синка и карточек ТС. Превысить нельзя —
# общий token-bucket (rate_limit) режет суммарную частоту всех воркеров на аккаунт.
TRACK_BACKFILL_RATE_PER_MIN = int(os.getenv("TRACK_BACKFILL_RATE_PER_MIN", "120") or 120)
# Пул воркеров бэкфилла: перекрывают сетевую латентность (~1-2с/трек), чтобы реально
# достигать потолка частоты, а не упираться в латентность однопотока. Лимитер всё равно
# держит суммарную частоту ≤ потолка — воркеры лишь утилизируют её.
TRACK_BACKFILL_WORKERS = int(os.getenv("TRACK_BACKFILL_WORKERS", "6") or 6)
